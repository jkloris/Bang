...
class Appaloosa extends BlueCard {
	...
	checkAppaloosa(game, player) {
		for (var i in game.players[player].blueCards) {
			if (game.players[player].blueCards[i].name == 'Appaloosa') return false;
		}
		return true;
	}
}
...
function discardBlueCard(game, player_i, card_i) {
	...
	if (game.players[player_i].blueCards[card_i].gun == true) game.players[player_i].scope.gun = 0;
