...
class Player {
	...
	loseHp(number) {
		this.HP = this.HP - number > 0 ? this.HP - number : 0;
	}
}

class Character {
	constructor(player) {
		//vlastnost player je asi zbytocna, ale nechce sa mi to odtranovat a ked ju odkomentujem tak sa to zacykli
		// this.player = player;
		this.name = null;
		this.HP = 4;
	}
...
class Pedro_ramirez extends Character {
	...
	action(game, player, io) {
		if (
			game.turn == player &&
			game.moveStage == 0 &&
			!game.players[player].dynamit &&
			!game.players[player].prison &&
			game.trashedCards > 0
		) {
			game.moveStage++;
			game.dealOneCard(player);
...
class Lucky_duke extends Character {
	...
	action(game, player, io) {
		if (player == game.turn && game.moveStage == 0) {
			...
			player == game.requestedPlayer &&
			game.requestedCard == 'Vedle' &&
			game.players[game.requestedPlayer].blueCards.length > 0 &&
			game.barelLimit > 0
		) {
...
class Kit_carlson extends Character {
	...
	action(game, index_sender, io) {
		if (
			game.turn == index_sender &&
			game.moveStage == 0 &&
			game.players[index_sender].prison == false &&
			game.players[index_sender].dynamit == false
		) {
...
class Greg_digger extends Character {
	...
	action(game, player, io, force) {
		if (force == true) {
			game.players[player].HP =
				game.players[player].HP + 2 > game.players[player].maxHP
					? game.players[player].maxHP
					: game.players[player].HP + 2;
			io.emit(
				'log',
