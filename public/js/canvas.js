...
function drawGame() {
	...
	//hrac co je na tahu
	if (
		game_client.turn != null &&
		game_client.requestedPlayer == null &&
		socket.id == game_client.players[game_client.turn].id
	) {
		...
		if (game_client.moveStage == 0) {
			...
			if (game_client.safeBeer > -1) game_scene.buttons[2].visible = false; //2 je use card
	...
	//hrac, co je na tahu, ale je v dueli
		...
		game_client.requestedPlayer != null &&
		socket.id == game_client.players[game_client.turn].id &&
		game_client.players[game_client.requestedPlayer].id == socket.id &&
		game_client.playedCard == 'Duel' &&
		game_client.safeBeer < 0
	) {
		ctx.save();
		...
		for (var i in game_scene.buttons) {
			if (i == 2 || i == 3) {
				//use button je na indexe 2 a lose life na 3
				game_scene.buttons[i].visible = true;
			} else {
	...
	//hrac, co nie je na tahu, ale je od neho vyzadovana nejaka akcia
		...
		game_client.requestedPlayer != null &&
		socket.id != game_client.players[game_client.turn].id &&
		socket.id == game_client.players[game_client.requestedPlayer].id
	) {
		if (game_client.safeBeer == game_client.requestedPlayer) {
			ctx.save();
			ctx.textAlign = 'center';
			ctx.fillStyle = 'red';
			ctx.font = '40px Arial';
			...
			ctx.restore();
			game_scene.buttons[2].visible = true;
	...
	if (game_client.started) {
		//kontroluje, ci uz bola zacata hra
		var x = 10;
		...
		ctx.drawImage(
			...
			y,
			ratio,
			(Sprites.bang.height / Sprites.bang.width) * ratio
		);
		...
		if (game_client.moveStage == 0 && index == game_client.turn && game_client.players[index].dynamit) {
			...
			game_client.moveStage == 0 &&
			index == game_client.turn &&
			game_client.players[index].prison == true
		) {
			ctx.save();
			ctx.textAlign = 'center';
			ctx.fillStyle = 'green';
			...
			ctx.restore();
		} else if (game_client.moveStage == 0 && index == game_client.turn && game_client.safeBeer < 0) {
			ctx.save();
			ctx.textAlign = 'center';
			ctx.fillStyle = 'green';
			ctx.font = '30px Arial';
			...
			ctx.restore();
		} else if (index == game_client.turn && game_client.safeBeer == index) {
			ctx.save();
			ctx.textAlign = 'center';
			ctx.fillStyle = 'red';
			ctx.font = '40px Arial';
		...
		if (game_client.playedCard == 'Hokynarstvo') {
			...
			while (turn != game_client.turn || count == 0) {
				turn++;
				if (turn >= game_client.players.length) turn = 0;
				if (game_client.players[turn].alive) count++;
			}

			for (var i = 0; i < count; i++) {
				game_client.cards[game_client.cards.length - i - 1].draw(
					canvas.width / 2 + (i + 1) * ratio - dl,
					canvas.height / 3.7,
					ratio
				);
			}
...
function kit_carlsonDraw() {
	...
	for (var i = 0; i < 3; i++) {
		game_client.cards[game_client.cards.length - i - 1].draw(
			canvas.width / 2 + (i + 1) * ratio - 3 * ratio,
			canvas.height / 2 - (Sprites.bang.height / Sprites.bang.width) * ratio,
			ratio
		);
	}
}
...
function lucky_dukeDraw() {
	...
	for (var i = 0; i < 2; i++) {
		game_client.cards[game_client.cards.length - i - 1].draw(
			canvas.width / 2 + (i + 1) * ratio - 2 * ratio,
			canvas.height / 2 - (Sprites.bang.height / Sprites.bang.width) * ratio,
			ratio
		);
	}
