...
document.addEventListener('contextmenu', (e) => {
	...
	if (game_scene.buttons[2].visible && cardSelected)
		game_scene.buttons[2].onclick(); //ak je selectnuta karta, tak rightclick = useCard.onclick();
	else if (!zoomed) game_scene.rightClickCheck(mouse); //ak nie je zoomnute
	else {
		//ak je zoomnute, tak sa zoomnutie zrusi a vykresli sa normalne hra
		zoomed = false;
		drawGame();
	}
});
...
socket.on('partial_clickAccept', (mouse) => {
	...
	var clickedBlue = game_scene.checkBlueCards(
		mouse,
...
socket.on('logClick', (point) => {
	...
	if (
		current.visible &&
		point.x >= current.x &&
		point.x <= current.x + current.sizeX &&
		point.y >= current.y &&
		point.y <= current.y + current.sizeY
	) {
		current.onclick();
	}
...
socket.on('emporio_clickAccept', (mouse) => {
	//TODO
...
function assignCardInstances(game_server) {
...
function updatePlayers(game_server) {
	...
	assignCardInstances(game_server);
	//YESKONECNE TOTO VYZERA ZE FUNGUJE
	setGameTiles(game_server);
}

function setGameTiles(game_server) {
	...
	for (number in game_client.players) {
		...
		switch (parseInt(number)) {
			...
			case 1:
				x = canvas.width / 18;
				y = canvas.height / 10 + tile_size.y * 1.5;
				break;
			case 2:
				x = canvas.width / 4.2;
				y = canvas.height / 2;
				break;
			case 3:
				x = canvas.width / 2 - tile_size.x / 2;
				y = canvas.height / 2 + 10;
				break;
			case 4:
				x = canvas.width - canvas.width / 2.6;
				y = canvas.height / 2;
				break;
			case 5:
				x = canvas.width - canvas.width / 18 - tile_size.x;
				y = canvas.height / 10 + tile_size.y * 1.5;
				break;
			case 6:
				x = canvas.width - canvas.width / 18 - tile_size.x;
		...
		var character =
			game_client.players[number].character == null ? null : game_client.players[number].character.name;
		var new_tile = new Tile(
			x,
			y,
			game_client.players[number].name,
			game_client.players[number].id,
			game_client.players[number].HP,
			game_client.players[number].cards.length,
			game_client.players[number].role,
			character
		);
...
function setName() {
	...
	if (name == null || name == 'null') {
		//ak tam nebolo ziadne meno, bud promptne nove, alebo sa pouzije arne birkenstock
