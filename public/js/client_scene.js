class Scene {
	...
	onclick = function (point) {
		...
		if (game_client.moveStage == 1 || (game_client.safeBeer > -1 && player_i == game_client.turn)) {
	...
	checkCharacters(point) {
		...
		if (
			point.x >= x &&
			point.x <= x + localRatio &&
			point.y >= y - (current.HP * (tile_size.y - 2)) / 10 + 1 &&
			point.y <=
				y - (current.HP * (tile_size.y - 2)) / 10 + 1 + (Sprites.bang.height / Sprites.bang.width) * localRatio
		) {
	...
	checkTiles(point) {
		for (var i in this.tiles) {
			...
			if (
				point.x >= current.x &&
				point.x <= current.x + tile_size.x &&
				point.y >= current.y &&
				point.y <= current.y + tile_size.y
			) {
	...
	checkBlueCards(point, current, player) {
		...
		for (var i in game_client.players[player].blueCards) {
			if (
				...
				point.x <= x + ratio / 1.7 &&
				point.y >= y &&
				point.y <= y + (Sprites.bang.height / Sprites.bang.width) * ratio
			) {
				return i;
			}
	...
	checkDeck(point) {
		...
		if (
			point.x >= current.x &&
			point.x <= current.x + ratio &&
			point.y >= current.y &&
			point.y <= current.y + (Sprites.bang.height / Sprites.bang.width) * ratio
		) {
	...
	checkEmporio(point) {
		...
		for (var i = 0; i < count; i++) {
			if (
				point.x >= canvas.width / 2 + (i + 1) * ratio - dl &&
				point.x <= ratio + canvas.width / 2 + ((i + 1) * ratio - dl) &&
				point.y >= canvas.height / 3.7 &&
				point.y <= canvas.height / 3.7 + (Sprites.bang.height / Sprites.bang.width) * ratio
			) {
	...
	kit_carlsonCheck(point) {
		...
		for (var i = 0; i < 3; i++) {
			if (
				point.x >= canvas.width / 2 - 3 * ratio + (i + 1) * ratio &&
				point.x <= -2 * ratio + canvas.width / 2 + (i + 1) * ratio &&
	...
	lucky_dukeCheck(point) {
		...
		for (var i = 0; i < 2; i++) {
			if (
				point.x >= canvas.width / 2 - ratio + i * ratio &&
				point.x <= canvas.width / 2 + i * ratio &&
				point.y >= canvas.height / 2 - (Sprites.bang.height / Sprites.bang.width) * ratio &&
				point.y <= canvas.height / 2
			) {
	...
	rightClickCheck(point) {
		...
		for (var i in game_client.players) {
			...
			if (
				...
				point.y <=
					y -
						(current.HP * (tile_size.y - 2)) / 10 +
						1 +
						(Sprites.bang.height / Sprites.bang.width) * localRatio
			) {
